...
# -----------------------------------------
# Find fraud transactions with history
# -----------------------------------------
...
# -----------------------------------------
# Evaluate each fraud transaction
# -----------------------------------------
...
for _, fraud_row in fraud_rows.iterrows():
    ...
    # Last 10 historical transactions
    sequence = previous[
        FEATURES
    ].tail(SEQUENCE_LENGTH).values.astype(
        np.float32
    )
    ...
    # Scale
    sequence = (
        sequence.reshape(1, SEQUENCE_LENGTH, len(FEATURES))
        - feature_mean
    ) / feature_std
...
# -----------------------------------------
# Highest probability fraud cases
# -----------------------------------------

print("\nTop 10 detected fraud cases:")
...
results_df.to_csv(
    "pre_fraud_evaluation_results.csv",
    index=False
)
